#!/bin/bash
# should be run as root and only on Debian 12 (Bookworm) version! (probably the next version is also possible)
...
if [[ $osVersion == *"bookworm"* ]]; then
    ...
    apt-get update && apt-get -y upgrade && apt-get install python3-venv python3-dev virtualenv redis-server postgresql nginx git gcc vim unzip imagemagick wget xz-utils ffmpeg lsb-release -y
else
