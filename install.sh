...
if [[ $osVersion == *"bookworm"* ]]; then
    ...
    apt-get update && apt-get -y upgrade && apt-get install python3-venv python3-dev virtualenv redis-server postgresql nginx vim unzip imagemagick wget xz-utils ffmpeg -y
else
